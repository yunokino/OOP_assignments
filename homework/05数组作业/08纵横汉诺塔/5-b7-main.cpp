...
void move(int n, char x, char y, char start, char end, int m)
{
	if (x == 'A' && y == 'B')
	{
		tmp[b++] = src[--a];
	...
	cct_gotoxy(0, 0);
	cout << "从 " << start << " 移动到 " << end << "，共 " << m << " 层，延时设置为 " << sleep << "，" << (display == 0 ? "不" : "") << "显示内部数组值\n";
	print();
	if (step == 1)
		cout << "第" << setw(4) << step << "步" << "(" << n << "#: " << x << "-->" << y << ") ";
	else
	{
		if (sleep == 0 && display == 1)
			_getch();
		cout << "第" << setw(4) << step << "步" << "(" << n << "#: " << x << "-->" << y << ") ";
	}
	step++;
...
{
	static int m = n;
	static char start = src;
	static char end = dst;
	if (n == 1)
	{
		move(1, src, dst, start, end, m);
	}
	else
	{
		hanoi(n - 1, src, dst, tmp);
		move(n, src, dst, start, end, m);
