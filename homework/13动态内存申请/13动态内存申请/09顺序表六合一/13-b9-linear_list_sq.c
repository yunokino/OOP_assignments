...
Status ClearList1(sqlist* L)
{
    ...
    {
        L->elem = (ElemType*)realloc(L->elem, LIST_INIT_SIZE);
        if (L->elem == NULL)
            exit(LOVERFLOW);
    }
    L->length = 0;
...
Status ListInsert1(sqlist* L, ElemType cur_e, ElemType e, Status(*compare)(ElemType e1, ElemType e2))
{
    ...
    while (i  < L->length + 1)
    {
        if((*compare)(L->elem[i - 1], cur_e) == OK)
            count++;
        i++;
    }
    if (count == 0)
    ...
    while (count != 0)
    {
        while ((*compare)(L->elem[i - 1], cur_e) == FALSE)
        {
            i++;
        }
        count--;
...
Status ListDelete1(sqlist* L, ElemType cur_e, Status(*compare)(ElemType e1, ElemType e2))
{
    ...
    p = &(L->elem[i - 1]); 		//指向第i个元素
    if (L->length <= L->listsize - LISTDECREMENT - LISTINCREMENT && L->length >= L->listsize - 2 * LISTINCREMENT)
    {
        L->elem = (ElemType*)realloc(L->elem, L->listsize - LISTINCREMENT);
        L->listsize = L->listsize - LISTDECREMENT;
    }
...
Status ListDelete2(sqlist* L, ElemType cur_e, Status(*compare)(ElemType e1, ElemType e2))
{
    ...
    L->length--;	//长度-1
    while (ListDelete2(L, cur_e, (*compare)) != ERROR)
        ;
    return OK;
}