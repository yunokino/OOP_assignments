...
{
	if (tmp.content == NULL || tmp.len == 0)
		content = NULL;
	else
	{
		content = new(nothrow) char[tmp.len + 1];
		if (content == NULL)
			exit(-1);
...
void TString:: set(const char* s)
{
	...
	if (s != NULL)
	{
		...
		TString::content = new(nothrow) char[i + 1];
		if (content == NULL)
			exit(-1);
		strcpy(TString::content, s);
		len = i;
	}
	else
	{
...
void TString:: set(TString& tmp)
{
	if (tmp.content == NULL || tmp.len == 0)
		content = NULL;
	else
	{
		TString::content = new(nothrow) char[tmp.len + 1];
		if (content == NULL)
			exit(-1);
		strcpy(TString::content, tmp.content);
	}
	len = tmp.len;
}
